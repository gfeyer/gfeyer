...
Systems programmer focused on **C++ and Go** — performance engineering, data serialization, and high-throughput services. I like finding the bug nobody else can reproduce and the allocation nobody knew was there.
...
- **[haproxy-spoe-go](https://github.com/negasus/haproxy-spoe-go/pull/28)** (Go) — Profiled a high-throughput SPOE agent and found `Frame.Read` responsible for ~75% of total allocations, with GC consuming ~16% of CPU. Reusing the read buffer across pooled frames cut frame-buffer allocations by **99%** and GC overhead to **~3%**.
...
- **[fleet_commander](https://github.com/gfeyer/fleet_commander)** (C++) — 2D colonization simulator. [Playable on itch.io](https://gfeyer.itch.io/fleet-commander).
...
Memory and allocation behavior · serialization formats and binary protocols · GC and latency tuning · game/simulation architecture (ECS)
